...
class MaterialService:
    ...
    def detect_all_capabilities(cls, musicxml_content: str, extraction_result=None) -> Set[str]:
        ...
        registry_caps = detection_engine.detect_all_capabilities(extraction_result)
    ...
    def compute_and_store_bitmasks(material: Material, bit_indices: List[int]):
        ...
        if bit_indices:
            bitmask_int = compute_capability_bitmask(bit_indices)
            material.capability_bitmask_low = bitmask_int & ((1 << 64) - 1)
            material.capability_bitmask_high = bitmask_int >> 64
        else:
            material.capability_bitmask_low = 0
            material.capability_bitmask_high = 0

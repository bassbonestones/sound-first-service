...
def handle_record_attempt(
    ...
    candidate = MaterialCandidate(
        material_id=material_id,
        ema_score=float(state.ema_score or 0.0),
        attempt_count=int(state.attempt_count or 0),
        status=MaterialStatus(str(state.status)) if state.status else MaterialStatus.UNEXPLORED
    )
    ...
    state.ema_score = result.new_ema  # type: ignore[assignment]
    state.attempt_count = result.new_attempt_count  # type: ignore[assignment]
    state.last_attempt_at = now  # type: ignore[assignment]
    state.status = result.new_status.value  # type: ignore[assignment]
    
    if is_off_course:
        state.manual_attempt_count = (int(state.manual_attempt_count or 0) + 1)  # type: ignore[assignment]
    else:
        state.guided_attempt_count = (int(state.guided_attempt_count or 0) + 1)  # type: ignore[assignment]
    ...
    attempt_id = int(attempt.id) if attempt.id is not None else 0
    update_capability_evidence(db, user_id, material_id, attempt_id, rating, 
...
def update_capability_evidence(
    ...
    for cap_id in capability_ids:
        ...
        if user_cap:
            user_cap.evidence_count = (int(user_cap.evidence_count or 0) + 1)  # type: ignore[assignment]


def update_capability_mastery(db: Session, user_id: int, capability_ids: List[int], now: datetime) -> None:
    ...
    for cap_id in capability_ids:
        ...
        if user_cap and not user_cap.mastered_at:
            ...
            if cap.bit_index is not None:
                set_user_capability_bit(db, user_id, int(cap.bit_index))
...
def check_soft_gate_requirements(db: Session, user_id: int, capability: Capability) -> bool:
    ...
    try:
        requirements = json.loads(cast(str, capability.soft_gate_requirements))
    ...
    for dimension_name, threshold in requirements.items():
        ...
        comfortable_value = float(state.comfortable_value) if state and state.comfortable_value else 0.0
...
def set_user_capability_bit(db: Session, user_id: int, bit_index: int) -> None:
...
def update_user_ability_scores(db: Session, user_id: int, material_id: int) -> None:
    ...
    for material_score, ability_col, stage_col in domains:
        ...
        current_ability = float(getattr(scores, ability_col) or 0.0)
        new_ability = max(current_ability, float(material_score))
        
        if new_ability > current_ability:
            setattr(scores, ability_col, new_ability)
            setattr(scores, stage_col, score_to_stage(new_ability))
    
    scores.updated_at = datetime.now()  # type: ignore[assignment]
...
def update_pitch_focus_stats(
    ...
    for context_type, context_id in contexts:
        ...
        stat.ema_score = compute_ema(float(rating), float(stat.ema_score or 0.0), config=config)  # type: ignore[assignment]
        stat.attempt_count = (int(stat.attempt_count or 0) + 1)  # type: ignore[assignment]
        stat.last_attempt_at = now  # type: ignore[assignment]
